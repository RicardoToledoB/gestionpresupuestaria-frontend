...
@Component({
  ...
  template: `
    <div class="ss-root" [class.ss-disabled]="disabled" [class.ss-open]="panelOpen">
      ...
      @if (panelOpen) {
        <div class="ss-panel" [ngStyle]="panelStyle" (click)="$event.stopPropagation()" (wheel)="$event.stopPropagation()" (touchmove)="$event.stopPropagation()">
          @if (allowNull) {
  ...
  styles: [`
    ...
    .ss-panel::-webkit-scrollbar { width: 10px; }
    .ss-panel::-webkit-scrollbar-thumb { background: rgba(102, 112, 133, .45); border-radius: 999px; border: 2px solid #fff; }
...
export class SearchSelectComponent implements ControlValueAccessor, OnChanges {
  ...
  private updatePanelPosition(): void {
    if (!this.panelOpen || typeof window === 'undefined') return;
    ...
    const control = this.host.nativeElement.querySelector('.ss-control') as HTMLElement | null;
    if (!control) return;
    ...
    const maxHeight = Math.max(minHeight, Math.min(desiredMax, openBelow ? spaceBelow : spaceAbove));
    ...
    this.panelStyle = {
      ...
      bottom: openBelow ? 'auto' : `${window.innerHeight - rect.top + 8}px`
    };
  }
