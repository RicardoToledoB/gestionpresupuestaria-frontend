...
export class CdpComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.buildForm();
    this.loadLookups();
    this.registerCoverageMonthCalculation();
    this.loadData();
  ...
  save(): void {
    ...
    const payload = this.normalize(this.form.getRawValue());
  ...
  private registerCoverageMonthCalculation(): void {
    const start = this.form.get('coverageStart');
    const end = this.form.get('coverageEnd');
    start?.valueChanges.subscribe(() => this.calculateCoverageMonths());
    end?.valueChanges.subscribe(() => this.calculateCoverageMonths());
    this.calculateCoverageMonths();
  }

  private calculateCoverageMonths(): void {
    const start = this.form.get('coverageStart')?.value as Date | null;
    const end = this.form.get('coverageEnd')?.value as Date | null;
    const months = this.calculateInclusiveCalendarMonths(start, end);
    const control = this.form.get('coverageMonths');
    if (control?.value !== months) {
      control?.setValue(months, { emitEvent: false });
    }
  }

  private calculateInclusiveCalendarMonths(start: Date | null, end: Date | null): number {
    if (!start || !end) return 0;
    const cleanStart = this.stripTime(start);
    const cleanEnd = this.stripTime(end);
    if (cleanEnd.getTime() < cleanStart.getTime()) return 0;
    return ((cleanEnd.getFullYear() - cleanStart.getFullYear()) * 12) + (cleanEnd.getMonth() - cleanStart.getMonth()) + 1;
  }
