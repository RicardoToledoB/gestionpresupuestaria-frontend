...
export class ImportsComponent implements OnInit {
  ...
  upload(): void {
    ...
    this.api.upload<ImportResult>('/imports/excel', this.selectedFile).subscribe({
      ...
      error: error => {
        ...
        this.loading = false;
        this.message = this.buildErrorMessage(error, 'No fue posible completar la importación.');
        this.uiRefresh.refresh(this.cdr);
      }
    });
  }
  ...
  clearDatabase(): void {
    ...
    this.api.delete<{ status: string; message: string }>('/imports/database').subscribe({
      ...
      error: error => {
        ...
        this.loading = false;
        this.message = this.buildErrorMessage(error, 'No fue posible limpiar la base H2.');
        this.uiRefresh.refresh(this.cdr);
      }
    });
  }
  private buildErrorMessage(error: any, fallback: string): string {
    ...
    return backendMessage ? `${fallback}${status}: ${backendMessage}` : `${fallback}${status}. Revise consola backend y Network del navegador.`;
  }
