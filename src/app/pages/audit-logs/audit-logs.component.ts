...
export class AuditLogsComponent implements OnInit {
  data: AuditLog[] = [];
  columns = ['createdAt', 'module', 'action', 'businessKey', 'entity', 'username', 'requestPath', 'observation', 'detail'];
  ...
  detailOf(row: AuditLog): string {
    const before = this.short(row.previousValue);
    const after = this.short(row.newValue);
    if (before && after) return `Antes: ${before} | Después: ${after}`;
    if (after) return `Nuevo: ${after}`;
    if (before) return `Anterior: ${before}`;
    return '-';
  }
