...
import { Observable, debounceTime, distinctUntilChanged, finalize, of, shareReplay, timeout } from 'rxjs';
...
export class PurchaseOrdersComponent implements OnInit, OnDestroy {
  ...
  openCeroPapelRequest(purchaseRequestId: string | null | undefined): void {
    ...
    this.api.post<any>('/ceropapel/purchase-requests/show', { id })
      .pipe(timeout(15000), finalize(() => this.ceroPapelLoadingIds.delete(id)))
      .subscribe({
        next: data => {
...
export class PurchaseRequestPickerDialogComponent {
  ...
  private getRequestById(id: string): Observable<any> {
    const cached = PurchaseRequestPickerDialogComponent.requestCache.get(id);
    if (cached) {
      console.info(`[CeroPapel Front] cache HIT solicitud ${id}`);
      return of(cached);
    }

    const existing = PurchaseRequestPickerDialogComponent.inFlightRequests.get(id);
    if (existing) {
      console.info(`[CeroPapel Front] reutilizando request en curso solicitud ${id}`);
      return existing;
    }

    const request$ = this.api.post<any>('/ceropapel/purchase-requests/show', { id })
      .pipe(
        timeout(10000),
        finalize(() => PurchaseRequestPickerDialogComponent.inFlightRequests.delete(id)),
        shareReplay(1)
      );

    PurchaseRequestPickerDialogComponent.inFlightRequests.set(id, request$);
    request$.subscribe({
      next: data => PurchaseRequestPickerDialogComponent.requestCache.set(id, data),
      error: () => {}
    });
    return request$;
  }
...
export class CdpDetailDialogComponent {
  ...
  openSolicitud(oc: AssociatedPurchaseOrder): void {
    ...
    this.api.post<any>('/ceropapel/purchase-requests/show', { id: String(oc.purchaseRequestId) }).pipe(timeout(15000)).subscribe({
