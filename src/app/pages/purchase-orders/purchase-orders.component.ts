...
import { Observable, debounceTime, distinctUntilChanged, finalize, map, of, shareReplay, timeout } from 'rxjs';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, CurrencyPipe, DatePipe, ReactiveFormsModule, MatButtonModule, MatCardModule, MatCheckboxModule, MatFormFieldModule, MatIconModule, MatInputModule, MatDatepickerModule, MatNativeDateModule, MatPaginatorModule, MatSnackBarModule, MatTableModule, MatTooltipModule, MatDialogModule, SearchSelectComponent],
...
export class PurchaseOrdersComponent implements OnInit, OnDestroy {
  ...
  ceroPapelLoadingIds = new Set<string>();
  ceroPapelRequestCache = new Map<string, any>();
  ...
  openCeroPapelRequest(purchaseRequestId: string | null | undefined): void {
    ...
    const cached = this.ceroPapelRequestCache.get(id);
    if (cached) {
      this.openCeroPapelDialog(cached);
      return;
    }

    if (this.ceroPapelLoadingIds.has(id)) {
      this.snack.open(`La solicitud CeroPapel ${id} ya se está consultando.`, 'Cerrar', { duration: 2500 });
      return;
    }

    this.ceroPapelLoadingIds.add(id);
    this.snack.open(`Consultando solicitud CeroPapel ${id}...`, 'Cerrar', { duration: 2000 });

    // IMPORTANTE:
    // En esta ventana NO usamos /purchase-requests/show porque en producción quedó
    // lento/intermitente para el modal. Se reutiliza exactamente la misma lógica
    // del módulo "Solicitudes de Compra": GET /ceropapel/purchase-requests?id=...
    // Ese flujo consulta el origen /purchase-requests/abastecimiento y retorna rápido por ID.
    this.getCeroPapelRequestFastById(id)
      .pipe(timeout(10000), finalize(() => this.ceroPapelLoadingIds.delete(id)))
      .subscribe({
        next: data => {
          this.ceroPapelRequestCache.set(id, data);
          this.openCeroPapelDialog(data);
        },
        error: err => this.snack.open(err?.error?.message ?? 'No fue posible consultar la solicitud en CeroPapel.', 'Cerrar', { duration: 6000 })
      });
  }

  private openCeroPapelDialog(data: any): void {
  ...
  private getCeroPapelRequestFastById(id: string): Observable<any> {
    return this.api.get<any>('/ceropapel/purchase-requests', {
      id,
      page: 0,
      size: 20
    }).pipe(
      map(response => {
        ...
        if (Array.isArray(response?.result)) candidates.push(...response.result);
        if (Array.isArray(response?.raw?.result)) candidates.push(...response.raw.result);
        if (response && !Array.isArray(response) && (response.id || response.raw || response.id_solicitud || response.idSolicitud)) candidates.push(response);
        const exact = candidates.find(item => String(item?.id ?? item?.raw?.id_solicitud ?? item?.id_solicitud ?? item?.idSolicitud ?? '').trim() === String(id).trim());
        const found = exact ?? candidates[0];
        if (found) return found;
        throw new Error(response?.message || `No se encontró la solicitud CeroPapel ${id}.`);
      })
    );
  }
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, CurrencyPipe, MatButtonModule, MatIconModule, MatDialogModule, MatFormFieldModule, MatInputModule, MatTooltipModule],
  template: `
...
export class PurchaseRequestPickerDialogComponent {
  private static readonly requestCache = new Map<string, any>();
  private static readonly inFlightRequests = new Map<string, Observable<any>>();
  ...
  search(): void {
    ...
    const cached = PurchaseRequestPickerDialogComponent.requestCache.get(id);
    if (cached) {
      this.request = cached;
      this.loading = false;
    ...
    // Se replica el flujo rápido de Solicitudes de Compra: GET /ceropapel/purchase-requests?id=...
    // Además se acepta cualquier forma de respuesta: PageResponse, result[], DTO directo o raw.result[].
    this.api.get<any>('/ceropapel/purchase-requests', { id, page: 0, size: 20 })
      .pipe(timeout(15000))
      .subscribe({
        next: response => {
          const data = this.extractRequestFromResponse(response, id);
          if (!data) {
            this.request = null;
            this.message = `No se encontró una solicitud válida con ID ${id}.`;
          } else {
            this.request = data;
            PurchaseRequestPickerDialogComponent.requestCache.set(id, data);
            this.message = 'Solicitud encontrada. Revise el resumen antes de vincularla al formulario de la OC.';
          }
          this.loading = false;
          this.cdr.detectChanges();
        ...
        error: err => {
          ...
          this.request = null;
          this.loading = false;
          this.message = err?.error?.message || err?.message || 'No fue posible consultar la solicitud en CeroPapel.';
          this.cdr.detectChanges();
        }
      });
  }

  private extractRequestFromResponse(response: any, id: string): any | null {
    const candidates: any[] = [];
    if (Array.isArray(response?.content)) candidates.push(...response.content);
    if (Array.isArray(response?.result)) candidates.push(...response.result);
    if (Array.isArray(response?.raw?.result)) candidates.push(...response.raw.result);
    ...
    const exact = candidates.find(item => String(this.requestId(item)).trim() === String(id).trim());
  ...
  totalEstimated(req: any): string { const raw = req?.raw ?? req ?? {}; const direct = req?.montoEstimado ?? raw?.montoEstimado ?? raw?.monto_estimado ?? raw?.total_estimado ?? raw?.totalEstimated; const directNumber = Number(String(direct ?? '').replace(/[^0-9.-]/g, '')); if (!Number.isNaN(directNumber) && directNumber > 0) return new Intl.NumberFormat('es-CL', { style:'currency', currency:'CLP', maximumFractionDigits:0 }).format(directNumber); const items = Array.isArray(raw.items) ? raw.items : (Array.isArray(raw.purchase_request_details) ? raw.purchase_request_details : []); const total = items.reduce((acc: number, item: any) => { const price = Number(String(item?.precio ?? item?.price ?? 0).replace(/[^0-9.-]/g, '')); const qty = Number(String(item?.cant ?? item?.cantidad ?? 1).replace(/[^0-9.-]/g, '')); return acc + (Number.isNaN(price) ? 0 : price * (Number.isNaN(qty) || qty <= 0 ? 1 : qty)); }, 0); return total > 0 ? new Intl.NumberFormat('es-CL', { style:'currency', currency:'CLP', maximumFractionDigits:0 }).format(total) : '-'; }
...
export class CdpDetailDialogComponent {
  ...
  openSolicitud(oc: AssociatedPurchaseOrder): void {
    ...
    const id = String(oc.purchaseRequestId);
    this.api.get<any>('/ceropapel/purchase-requests', { id, page: 0, size: 20 }).pipe(
      timeout(10000),
      map(page => {
        const content = Array.isArray(page?.content) ? page.content : [];
        if (content.length > 0) return content[0];
        throw new Error(page?.message || `No se encontró la solicitud CeroPapel ${id}.`);
      })
    ).subscribe({
