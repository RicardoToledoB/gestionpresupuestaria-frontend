...
export class PurchaseOrdersComponent implements OnInit {
  ...
  openMercadoPublico(row: PurchaseOrder): void {
    const code = String(row.orderNumber ?? '').trim();
    if (!code) {
      this.snack.open('La OC no tiene número válido para consultar Mercado Público.', 'Cerrar', { duration: 3500 });
      return;
    }
    this.snack.open(`Consultando Mercado Público: ${code}`, 'Cerrar', { duration: 1800 });
    this.api.get<MercadoPublicoPurchaseOrder>(`/mercado-publico/purchase-orders/${encodeURIComponent(code)}`).subscribe({
      ...
      error: err => {
        console.error(err);
        this.snack.open(err?.error?.message ?? 'No fue posible consultar la OC en Mercado Público. Revise ticket/API.', 'Cerrar', { duration: 6000 });
      }
    });
  }
...
@Component({
  ...
  standalone: true,
  imports: [CurrencyPipe, DatePipe, JsonPipe, MatButtonModule, MatIconModule, MatDialogModule],
  template: `
    <div class="mp-dialog">
      ...
      <mat-dialog-content>
            ...
            <div class="mp-items-list">
              @for (item of data.items; track $index) {
                <div class="mp-item-card">
                  <strong>{{ item['Nombre'] || item['Producto'] || item['Descripcion'] || item['EspecificacionComprador'] || 'Ítem ' + ($index + 1) }}</strong>
                  <pre>{{ item | json }}</pre>
                </div>
              }
            </div>
        ...
        <section class="mp-panel">
          ...
          <p class="mp-muted">Se deja disponible para revisión y trazabilidad técnica. La fuente es la API pública de Mercado Público.</p>
  ...
  styles: [`
    ...
    .mp-dialog-header h2 { margin: 0; font-size: 1.45rem; font-weight: 800; }
    .mp-dialog-header p { margin: 6px 0 0; color:#64748b; }
    ...
    dd { margin:0; }
    .mp-items { margin-bottom:16px; }
    .mp-items-list { display:grid; gap:10px; }
    .mp-item-card { background:#fff; border:1px solid #e5e7eb; border-radius:12px; padding:12px; }
    .mp-item-card pre, .mp-raw { white-space:pre-wrap; word-break:break-word; background:#0f172a; color:#dbeafe; padding:12px; border-radius:12px; max-height:260px; overflow:auto; font-size:.78rem; }
    .mp-muted { color:#64748b; margin-top:-4px; }
    @media (max-width: 760px) { .mp-kpi-grid, .mp-section-grid { grid-template-columns: 1fr; } dl { grid-template-columns: 1fr; } }
  `]
})
